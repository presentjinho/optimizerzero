...
const intentPresets = {
  ...
  share: {
    ...
    quality: 88,
    limit: 75,
    message: "공유용 추천: 균형 압축, 낮은 손실, 품질 88.",
  },
  messenger: {
    profile: "strong",
    lossBudget: "high",
    quality: 70,
    limit: 25,
    message: "메신저용 추천: 용량 우선, 품질 확인 후 사용.",
  },
  email: {
    profile: "balanced",
    lossBudget: "medium",
    quality: 82,
    limit: 25,
    message: "이메일 추천: 첨부 제한을 의식한 중간 손실 압축.",
  },
  quality: {
    profile: "balanced",
    lossBudget: "low",
    quality: 92,
    limit: 150,
    message: "품질 우선 추천: 거의 티 안 나게, 절감이 작으면 스킵.",
  },
};
...
function setFiles(files) {
  ...
  for (const file of files) {
    if (!supported(file)) {
      state.rejected.push({ name: file.name, reason: "unsupported" });
...
async function optimizeImageFile(file) {
  ...
  if (!blob || blob.size >= file.size) return { status: "skipped", message: "not smaller" };
  const outName = file.name.replace(/(\.[^.]+)?$/, ".ozero.webp");
  return { status: "optimized", blob, outName, saved: file.size - blob.size };
}
...
async function optimizeArchive(file) {
  ...
  for (const [name, entry] of Object.entries(source.files)) {
    ...
    if (cleanName.includes("../") || cleanName.startsWith("..")) throw new Error(`unsafe path: ${name}`);
  ...
  if (blob.size >= file.size) return { status: "skipped", message: "not smaller" };
  const outName = file.name.replace(/(\.[^.]+)$/, ".ozero$1");
  return { status: "optimized", blob, outName, saved: file.size - blob.size, message: "container recompressed" };
}
