...
## 3-1. 현재 적용된 임시 비공개 잠금

Access 설정 전까지는 Cloudflare Pages Function Basic Auth를 쓴다.

- [x] `functions/_middleware.js` 추가
- [x] `OPTIMIZERZERO_USER` Cloudflare Pages secret 설정
- [x] `OPTIMIZERZERO_PASSWORD` Cloudflare Pages secret 설정
- [x] 비밀번호 없는 요청은 401 확인
- [x] 틀린 비밀번호는 401 확인
- [x] 맞는 비밀번호는 200 확인

주의: 비밀번호는 코드, README, issue, commit에 적지 않는다.
